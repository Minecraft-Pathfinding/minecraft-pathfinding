...
export abstract class Movement {
  ...
  getBlockInfoBB(pos: Vec3Properties, dx: number, dy: number, dz: number, halfwidth: number=0.3, height: number=1.8) {
    const vertices = new AABB(pos.x+dx-halfwidth, pos.y+dy, pos.z+dz-halfwidth, pos.x+dx+halfwidth, pos.y+dy+height, pos.z+dz+halfwidth).toVecs();
  ...
  getBlockInfoPlane(pos: Vec3Properties, dx: number, dy: number, dz: number, halfwidth: number=0.3) {
    const vertices = [new Vec3(pos.x+dx-halfwidth, pos.y+dy, pos.z+dz-halfwidth), new Vec3(pos.x+dx+halfwidth, pos.y+dy, pos.z+dz+halfwidth), new Vec3(pos.x+dx-halfwidth, pos.y+dy, pos.z+dz+halfwidth), new Vec3(pos.x+dx+halfwidth, pos.y+dy, pos.z+dz-halfwidth)]
    return new BlockInfoGroup(...vertices.map((v) => this.world.getBlockInfo(v)))
    // return this.world.getBlockInfo(new Vec3(pos.x+dx, pos.y+dy, pos.z+dz))
  }
