...
class Movements {
  ...
  blocksCantBreak: Set<number> = new Set()
  blocksToAvoid: Set<number> = new Set()
  liquids: Set<number> = new Set()
  gravityBlocks: Set<number> = new Set()
  climbables: Set<number> = new Set()
  emptyBlocks: Set<number> = new Set()
  replaceables: Set<number> = new Set()
  scafoldingBlocks: number[] = []
  fences: Set<number> = new Set()
  carpets: Set<number> = new Set()
  openable: Set<number> = new Set()
  canOpenDoors = false
  maxDropDown = 4
  ...
  constructor (bot: Bot) {
    ...
    (registry.blocksArray as any[]).forEach((block: any) => {
    ...
    ;(registry.blocksArray as any[]).map(x => Block.fromStateId(x.minStateId, 0)).forEach(block => {
  ...
  getBlock (pos: Vec3, dx: number, dy: number, dz: number) {
  ...
  safeToBreak (pos: Vec3) {
    ...
    if (this.dontCreateFlow) {
      ...
      if (this.getBlock(pos, 0, 1, 0).liquid) return false
      if (this.getBlock(pos, -1, 0, 0).liquid) return false
      if (this.getBlock(pos, 1, 0, 0).liquid) return false
      if (this.getBlock(pos, 0, 0, -1).liquid) return false
      if (this.getBlock(pos, 0, 0, 1).liquid) return false
    }

    if (this.dontMineUnderFallingBlock) {
      // TODO: Determine if there are other blocks holding the entity up
      if (this.getBlock(pos, 0, 1, 0).canFall) {
        return false
      }
    }

    const block = this.getBlock(pos, 0, 0, 0)
   ...
   * @returns {number}
   */
  safeOrBreak (pos: Vec3, toBreak: Vec3[]) {
    let cost = 0
    const block = this.getBlock(pos, 0, 0, 0)
    if (block.safe) return cost
    if (!this.safeToBreak(pos)) return 100 // Can't break, so can't move
    toBreak.push(pos.clone())

    // TODO: port bestHarvestTool I guess
  ...
  getMoveJumpUp (node: NonePhysicsMove, dir: Vec3, neighbors: NonePhysicsMove[]) {
    const pos = new Vec3(node.x, node.y, node.z)
    const blockA = this.getBlock(pos, 0, 2, 0)
    const blockH = this.getBlock(pos, dir.x, 2, dir.z)
    const blockB = this.getBlock(pos, dir.x, 1, dir.z)
    const blockC = this.getBlock(pos, dir.x, 0, dir.z)

    let cost = 2 // move cost (move+jump)
    const toBreak = []
    const toPlace: PlaceInteraction[] = []

    if (blockA.physical) return // Blocks A, B and H are above C, D and the player's space, we need to make sure there are no entities that will fall down onto our building space if we break them
    if (blockH.physical) return
    if (blockB.physical && !blockH.physical && !blockC.physical) return // It is fine if an ent falls on B so long as we don't need to replace block C

    if (!blockC.physical) {
      // if (node.remainingBlocks === 0) return // not enough blocks to place

      const blockD = this.getBlock(pos, dir.x, -1, dir.z)
      if (!blockD.physical) {
        ...
        toPlace.push(new PlaceInteraction(blockC.position.x, blockC.position.y, blockC.position.z, dir.x, 0, dir.z, new Vec3(node.x, node.y, node.z)))
    ...
    const block0 = this.getBlock(pos, 0, -1, 0)
    ...
    neighbors.push(new NonePhysicsMove(blockB.position.x, blockB.position.y, blockB.position.z, cost, toBreak, toPlace))
  }

  getMoveForward (node: NonePhysicsMove, dir: Vec3, neighbors: NonePhysicsMove[]) {
    const pos = node.toVec3()
    const blockB = this.getBlock(pos, dir.x, 1, dir.z)
    const blockC = this.getBlock(pos, dir.x, 0, dir.z)
    const blockD = this.getBlock(pos, dir.x, -1, dir.z)
    ...
    if (this.getBlock(pos, 0, 0, 0).liquid) cost += this.liquidCost

    neighbors.push(new NonePhysicsMove(blockC.position.x, blockC.position.y, blockC.position.z, cost, toBreak, toPlace))
  }
