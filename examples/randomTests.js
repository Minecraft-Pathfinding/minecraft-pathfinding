...
async function cmdHandler(username, msg) {
  ...
  const author = bot.players[username];
  switch (cmd) {

    case "test": {

        let oppDir = bot.entity.position.minus(author.entity.position);

        // rotate oppDir 90 degrees
        const temp = oppDir.x;
        oppDir.x = -oppDir.z;
        oppDir.z = temp;

        oppDir = oppDir.scaled(10).plus(author.entity.position);

        bot.lookAt(oppDir, true);

        while (bot.entity.position.distanceTo(author.entity.position) > 0.3) {
            botSmartMovement(bot, author.entity.position, true);
            botStrafeMovement(bot, author.entity.position);
